...
describe("literal", function()
  describe("eval", function()
    it("reads scalars", function()
      assert.equal("x", eval_expression([["x"]]))
      assert.equal(3, eval_expression("3"))
      assert.equal(true, eval_expression("true"))
      assert.equal(false, eval_expression("false"))
    end)

    it("reads negative numbers", function()
      assert.equal(-2, eval_expression("-2"))
    end)

    it("folds literal concatenation, which rockspecs use constantly", function()
      assert.equal("ab", eval_expression([["a" .. "b"]]))
    end)
  ...
  describe("assignments", function()
    it("collects top-level globals as a rockspec writes them", function()
      ...
      assert.equal("privata", data.package)
      assert.equal("lua/privata/init.lua", data.build.modules["privata"])
    end)
  ...
  describe("returned_table", function()
    it("reads the table a config file returns", function()
      ...
      assert.equal("_Q", data.namespace)
    end)
