...
describe("parser", function()
  describe("statements", function()
    ...
    it("parses local function", function()
      ...
      assert.equal("LocalFunction", node.kind)
      assert.equal("helper", node.name.name)
      assert.equal("FunctionExpr", node.func.kind)
    end)

    it("parses assignment to several targets", function()
      local node = first("a, b.c = 1, 2")
      assert.equal("Assignment", node.kind)
      assert.equal(2, #node.targets)
      assert.equal("Index", node.targets[2].kind)
    end)
    ...
    it("parses if/elseif/else as clauses plus an else body", function()
      ...
      assert.equal("IfStatement", node.kind)
      assert.equal(2, #node.clauses)
      assert.equal(1, #node.else_body)
    end)
    ...
    it("parses both for shapes", function()
      assert.equal("NumericFor", first("for i = 1, 10 do end").kind)
      assert.equal("NumericFor", first("for i = 1, 10, 2 do end").kind)
      local generic = first("for k, v in pairs(t) do end")
      assert.equal("GenericFor", generic.kind)
      assert.same({ "k", "v" }, { generic.names[1].name, generic.names[2].name })
    end)

    it("parses while, repeat and do", function()
      assert.equal("WhileStatement", first("while a do end").kind)
      assert.equal("RepeatStatement", first("repeat until a").kind)
      assert.equal("DoStatement", first("do end").kind)
    end)

    it("parses return with and without values", function()
      assert.equal(0, #first("return").values)
      assert.equal(2, #first("return a, b").values)
    end)
    ...
    it("parses labels and goto", function()
      ...
      assert.equal("LabelStatement", statements[1].kind)
      assert.equal("GotoStatement", statements[2].kind)
      assert.equal("top", statements[2].label)
    end)
    ...
    it("still treats goto as a name where 5.1 allows it", function()
      ...
      assert.equal("Assignment", first("goto = 1").kind)
      assert.equal("CallStatement", first("goto()").kind)
    end)
  end)
  ...
  describe("functions", function()
    it("builds a dotted declaration target as an index chain", function()
      ...
      assert.equal("FunctionDeclaration", node.kind)
      assert.equal("a.b.c", ast.dotted_name(node.target))
    ...
    it("folds a method declaration into the same target shape", function()
      ...
      assert.equal("C.m", ast.dotted_name(node.target))
    ...
    it("records the closing line", function()
      ...
      assert.equal(1, node.func.line)
      assert.equal(3, node.func.end_line)
    end)
  end)
  ...
  describe("expressions", function()
    ...
    it("distinguishes unary from binary ~", function()
      assert.equal("UnaryOp", first("return ~a").values[1].kind)
      assert.equal("BinaryOp", first("return a ~ b").values[1].kind)
    end)
    ...
    it("keeps parentheses, which are semantic in Lua", function()
      ...
      assert.equal("Paren", node.kind)
      assert.equal("Call", node.expr.kind)
    end)

    it("parses all three call-argument forms", function()
      assert.equal("Call", first("f(1)").expr.kind)
      assert.equal("String", first("f 'x'").expr.args[1].kind)
      assert.equal("TableExpr", first("f { a = 1 }").expr.args[1].kind)
    end)

    it("parses method calls", function()
      local node = first("obj:run(1)").expr
      assert.equal("MethodCall", node.kind)
      assert.equal("run", node.method)
      assert.equal("obj", node.object.name)
    end)
    ...
    it("chains suffixes", function()
      ...
      assert.equal("Index", node.kind)
      assert.equal("e", node.index.value)
      assert.equal("MethodCall", node.object.kind)
      assert.equal("d", node.object.method)
      assert.equal("Index", node.object.object.kind)
  ...
  describe("tables", function()
    it("parses the three field forms", function()
      ...
      assert.equal(3, #node.fields)
      assert.is_nil(node.fields[1].key)
      assert.equal("x", node.fields[2].key.value)
    ...
    it("accepts semicolons and a trailing separator", function()
      ...
      assert.equal(2, #node.fields)
    end)

    it("parses an empty table", function()
      assert.equal(0, #first("return {}").values[1].fields)
    end)
  end)
